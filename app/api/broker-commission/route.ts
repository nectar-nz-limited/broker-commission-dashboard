...
export async function GET(request: Request) {
  ...
  try {
    ...
    const response = await fetch("https://bigquery.googleapis.com/bigquery/v2/projects/nectar-marketing-insights/queries", { method: "POST", headers: { authorization: `Bearer ${token.token}`, "content-type": "application/json" }, body: JSON.stringify({ query, useLegacySql: false }) });
    const body = await response.json() as any;
    if (!response.ok) throw new Error(body?.error?.message || "BigQuery query failed");
    const fields = body.schema?.fields || [];
    const rows = (body.rows || []).map((row: any) => Object.fromEntries(fields.map((field: any, index: number) => [field.name, row.f?.[index]?.v ?? null])));
