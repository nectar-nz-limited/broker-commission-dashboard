...
function UnderlyingData({ month }: { month: string }) {
  ...
  const downloadWorkbook = () => {
    ...
    const commissionData = commissionRows.map((x) => [x.application, x.broker, x.funding_date, Number(x.amount_funded || 0), Number(x.broker_fee || 0)]);
    const clawbackData = clawbackRows.map((x) => { const base = Number(x.commission_base || 0); const rate = historicalCommissionRate(x.broker, x.funding_date, base); return [x.application, x.broker, x.funding_date, x.paid_off_date, base, rate, base * rate]; });
    const waiverData = waiverRows.map((x) => [x.application, x.broker, x.funding_date, Number(x.incremental_nrw || 0), Number(x.waiver_commission || 0)]);
    const xml = `<?xml version="1.0"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet">${sheet("Commission", ["Application", "Broker", "Funding date", "Amount funded", "Broker fee"], commissionData)}${sheet("Clawback", ["Application", "Broker", "Funding date", "Paid-off date", "Commission base", "Historic commission rate", "Calculated clawback"], clawbackData)}${sheet("Waiver commission", ["Application", "Broker", "Funding date", "Incremental NRW", "Waiver commission"], waiverData)}</Workbook>`;
    const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([xml], { type: "application/vnd.ms-excel" })); a.download = `underlying-data-${monthKey}${week !== "All weeks" ? `-${week}` : ""}.xls`; a.click();
  };
  return <div><div className="table-tools"><button className="save" onClick={downloadWorkbook}>Download all 3 sheets</button></div><div className="table-tools"><label>Broker <select value={broker} onChange={(e) => { setBroker(e.target.value); if (brokerFrequency(e.target.value) !== "Weekly") setWeek("All weeks"); }}><option>All brokers</option>{brokerOptions.map((b) => <option key={b}>{b}</option>)}</select></label><label>Week <select value={week} disabled={broker !== "All brokers" && brokerFrequency(broker) !== "Weekly"} onChange={(e) => setWeek(e.target.value)}><option>All weeks</option>{weekOptions.map((w) => <option key={w} value={w}>{`Week commencing ${w}`}</option>)}</select></label><span className="sub">Monthly brokers are restricted to the selected month. Weekly brokers show all Monday–Sunday weeks in that month, or the selected week.</span></div><Panel title="Commission underlying data" action="Live BigQuery"><div className="table-wrap"><table><thead><tr><th>Application</th><th>Broker</th><th>Funding date</th><th>Amount funded</th><th>Broker fee</th></tr></thead><tbody>{commissionRows.map((x) => <tr key={x.application}><td>{x.application}</td><td>{x.broker}</td><td>{x.funding_date}</td><td>{Number(x.amount_funded || 0).toFixed(2)}</td><td>{Number(x.broker_fee || 0).toFixed(2)}</td></tr>)}</tbody></table></div></Panel><Panel title="Clawback underlying data" action="Historic rate basis"><div className="table-wrap"><table><thead><tr><th>Application</th><th>Broker</th><th>Funding date</th><th>Paid-off date</th><th>Commission base</th><th>Historic rate</th><th>Clawback</th></tr></thead><tbody>{clawbackRows.map((x) => { const base = Number(x.commission_base || 0); const rate = historicalCommissionRate(x.broker, x.funding_date, base); return <tr key={x.application}><td>{x.application}</td><td>{x.broker}</td><td>{x.funding_date}</td><td>{x.paid_off_date}</td><td>{base.toFixed(2)}</td><td>{(rate * 100).toFixed(2)}%</td><td>{(base * rate).toFixed(2)}</td></tr>; })}</tbody></table></div></Panel><Panel title="NRW waiver underlying data" action="35% waiver rate"><div className="table-wrap"><table><thead><tr><th>Application</th><th>Broker</th><th>Funding date</th><th>Incremental NRW</th><th>Waiver commission</th></tr></thead><tbody>{waiverRows.map((x) => <tr key={x.application}><td>{x.application}</td><td>{x.broker}</td><td>{x.funding_date}</td><td>{Number(x.incremental_nrw || 0).toFixed(2)}</td><td>{Number(x.waiver_commission || 0).toFixed(2)}</td></tr>)}</tbody></table></div></Panel></div>;
}
function UnderlyingDataLegacy({ month }: { month: string }) {
  ...
  const weeks = useMemo(
    () => [
      ...
      ...Array.from(
        ...
        ]).filter((name) => String(name || "").trim() !== "Direct"),
      )
  ...
  const matchPeriod = (name: string, date: string) => {
    const isWeekly = weeklyBrokers.has(String(name || "").trim());
    return isWeekly
